# Note-taking App — DevOps Assignment

This repository contains the Note-taking App used for the IE University BCSAI SDDO 2025 assignment. The project includes CI/CD pipelines, Terraform infrastructure for Azure Container Apps, and automated tests with coverage reporting.

Summary
- CI: runs unit/integration tests, enforces >= 70% coverage, builds Docker images and pushes them to an existing Azure Container Registry (ACR).
- CD: runs Terraform in `terraform/azure` to provision PostgreSQL Flexible Server and Azure Cache for Redis, a Container Apps environment, and three Container Apps (`frontend-notetakingapp`, `backend-notetakingapp`, `worker-notetakingapp`).
- Monitoring: basic `/health` endpoint and Prometheus metric exposition (minimal).

Quickstart (run tests & generate coverage HTML)

1. Create a Python virtualenv and install dev requirements:
...
2. Run the test suite and generate coverage HTML (the command below mirrors the CI):

```bash
# ensure the backend package is importable
export PYTHONPATH="${PWD}/backend:${PYTHONPATH}"

# run tests and generate reports under reports/
pytest tests \
  --cov=core \
...
python -m http.server -d reports/htmlcov 8000
# then visit http://localhost:8000
```

Notes
- The tests expect a PostgreSQL service for integration tests. The easiest approach is to use the GitHub Actions service container (in CI) or run the local DB with Docker Compose:

```bash
docker compose up -d db
# then run the pytest command above
```

- Ensure the following environment variables (or their GitHub Secrets) are set when running tests locally if your tests rely on them:
  - `DJANGO_SECRET_KEY`
  - `GOOGLE_API_KEY`
  - `PINECONE_API_KEY`

CI & CD
- CI workflow: `.github/workflows/ci.yml` (tests, builds, pushes images to ACR). It reconstructs `.env` from individual secrets.
- CD workflow: `.github/workflows/cd.yml` (triggers on CI success and supports manual dispatch). It runs Terraform in `terraform/azure` and deploys Container Apps.

Terraform
- Terraform code is under `terraform/azure`. The module references an existing ACR via `acr_login_server` and creates Postgres/Redis and Container Apps. Configure `TF_STATE_*` variables in GitHub and ensure `AZURE_CREDENTIALS` SP has permission to the state storage account.

Report and submission
- Run the tests locally and commit the files under `reports/` (JUnit XML, coverage XML, and HTML coverage) to include in your assignment submission.

If you want, I can create a short PR message and commit the `reports/` content after you generate it locally.
